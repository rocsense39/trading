...
def build_report(config_path: str | Path = "config/portfolio.json") -> str:
    ...
    # TODO: replace static regime with dynamic regime module in V5.3.
    regime = "RISK ON"
    regime_score = 90
    ...
    lines: list[str] = []
    lines.append("📊 ETF Bot V5.2")
    lines.append("Quiet data + allocation + scoring + SL/TP")
    lines.append("")
    ...
    for c in ranked:
        lines.append(
            f"{decision_icon(c.decision)} {c.key:<8} "
            f"{c.score:>3}/100 | {c.decision:<4} | confidence {c.confidence}%"
        )
    lines.append("")

    buy = next((c for c in ranked if c.decision == "BUY"), None)
    if not buy:
        lines.append("⚪ No BUY candidate today.")
        lines.append("")
        lines.append("Reason: no ETF passed allocation, scoring, and sizing filters.")
        return "\n".join(lines)
    ...
    lines.append(f"Decision: {buy.decision}")
    lines.append(f"Score: {buy.score}/100")
    ...
    lines.append(
        f"Reward/risk to TP1: {fmt(plan.reward_risk) if plan.reward_risk else 'n/a'}"
    )
    lines.append("")
    ...
    for r in rows:
        lines.append(
            f"{r.key:<8} target {r.target_weight:>5.1%} | "
            f"actual {r.actual_weight:>5.1%} | gap {r.gap_eur:+.2f} EUR"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--telegram",
        action="store_true",
        help="Send report to Telegram if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID are set",
    )
