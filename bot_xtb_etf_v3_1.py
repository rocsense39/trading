"""
XTB ETF Bot V3.1.1 — Allocation + SL/TP Discipline
=================================================
...
def score_candidate(meta: dict[str, Any], snap: MarketSnapshot, regime: str, regime_score: int, gap_eur: float, total_eur: float) -> tuple[int, list[str]]:
    """
    V3.1 portfolio-first scoring.
    This replaces the old all-or-nothing EMA150 blocking.

    Score components:
    - allocation gap: the portfolio need comes first
    - regime: macro permission
    - trend/momentum: technical sanity check
    - sleeve-specific tolerance: core and quality are allowed earlier than satellites
    """
    sleeve = meta.get("sleeve", "satellite")
    score = 0
    notes: list[str] = []

    gap_pct_value = gap_eur / total_eur if total_eur > 0 else 0.0

    # 1) Allocation gap: underweight ETFs get priority
    if gap_eur <= 0:
        return 0, ["not underweight"]
    if gap_pct_value >= 0.08:
        score += 35
        notes.append("large allocation gap")
    elif gap_pct_value >= 0.04:
        score += 28
        notes.append("medium allocation gap")
    elif gap_pct_value >= 0.015:
        score += 20
        notes.append("small allocation gap")
    else:
        score += 10
        notes.append("tiny allocation gap")
    ...
    if regime_score >= 80:
        ...
        score += 18
        notes.append("positive regime")
    elif regime_score >= 40:
        score += 8
        notes.append("neutral regime")
    else:
        if sleeve in {"core", "quality"}:
            score += 2
            notes.append("risk-off but core/quality allowed only cautiously")
        else:
            return 0, ["satellite blocked in risk-off"]

    # 3) Technical structure
    ema50_rising_proxy = snap.ema20 > snap.ema50 or snap.close > snap.ema50
    if snap.close > snap.ema20 > snap.ema50:
        score += 20
        notes.append("strong short-term trend")
    elif snap.close > snap.ema50 and ema50_rising_proxy:
        score += 15
        notes.append("above EMA50")
    elif snap.close > snap.ema150:
        score += 9
        notes.append("above EMA150")
    else:
        # V3.1: core growth is not hard-blocked by EMA150 when regime is very strong,
        # but the score is lower. Satellites remain stricter.
        if sleeve in {"core", "core_growth"} and regime_score >= 80:
            score += 5
            notes.append("below EMA150, but strong regime allows cautious core accumulation")
        elif sleeve == "quality" and regime_score >= 40:
            score += 7
            notes.append("quality sleeve allowed despite weak chart")
        else:
            score -= 10
    ...
    if snap.rsi14 >= 55:
        score += 10
        notes.append("RSI supportive")
    elif snap.rsi14 >= 48:
        score += 5
        notes.append("RSI neutral-positive")
    else:
        score -= 5
        notes.append("RSI weak")

    # 5) Sleeve thresholds
    # Core/quality should accumulate; satellites must earn their place.
...
def can_buy(meta: dict[str, Any], snap: MarketSnapshot, regime: str) -> tuple[bool, str]:
    # Kept only for compatibility if called elsewhere.
    sleeve = meta.get("sleeve", "satellite")
    if sleeve in {"core", "core_growth"}:
    ...
    if snap.close < snap.ema50:
        return False, "satellite below EMA50"
...
def choose_best_candidate(config: dict[str, Any], state: dict[str, Any], regime: str, regime_score: int) -> tuple[str, dict[str, Any]] | None:
    ...
    for name, item in view.items():
        ...
        score, notes = score_candidate(meta, snap, regime, regime_score, float(item["gap_eur"]), total)
        item["candidate_score"] = score
        item["score_notes"] = notes
        item["total_eur"] = total

        sleeve = meta.get("sleeve", "satellite")
        threshold = 60 if sleeve in {"core", "core_growth"} else 50 if sleeve == "quality" else 70

        print(f"{name}: score={score} threshold={threshold} — {'; '.join(notes)}")

        if score < threshold:
            continue

        # Ranking combines portfolio need and score. This prevents a tiny ETF with a tiny gap
        # from beating a core ETF that is materially underweight.
        rank = float(item["gap_eur"]) + score * 0.25
...
def make_order_message(config: dict[str, Any], state: dict[str, Any], name: str, item: dict[str, Any], regime: str, score: int, details: str) -> str | None:
    ...
    if not rr_ok:
        print(f"{name}: rejected — reward/risk {rr:.2f} below 1.50")
        return None
    ...
    if order_eur < float(config["settings"].get("min_order_eur", 25.0)):
        print(
            f"{name}: rejected — order {order_eur:.2f} EUR below min_order_eur "
            f"{float(config['settings'].get('min_order_eur', 25.0)):.2f}; "
            f"free_cash={free_cash:.2f}, reserve={reserve:.2f}, deployable={deployable:.2f}, gap={float(item['gap_eur']):.2f}"
        )
        return None
    ...
    msg = (
        f"📌 <b>ETF BOT V3.1 — ACTION REQUIRED</b>\n"
...
def portfolio_report(config: dict[str, Any], regime: str, score: int, details: str) -> str:
    view, total = build_portfolio_view(config)
    lines = [
        "📊 <b>ETF BOT V3.1 — PORTFOLIO STATUS</b>",
...
def run_once(config: dict[str, Any], state: dict[str, Any]) -> None:
    ...
    choice = choose_best_candidate(config, state, regime, score)
    if choice is None:
...
def startup_message(config: dict[str, Any]) -> str:
    lines = ["✅ <b>ETF Bot V3.1 started</b>", "Targets:"]
    for name, meta in config["etfs"].items():
...
def run() -> None:
    ...
    while True:
        ...
        try:
            ...
            send_telegram(f"🔴 <b>ETF Bot V3.1 error</b>\n{exc}")
